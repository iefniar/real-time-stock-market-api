...
import { generateToken, encryptToken, decryptToken } from '../lib/security/delete-token.js';
import crypto from 'crypto';
// Returns the user's existing token. If none exists, a new one is created.
export async function getOrCreateDeleteToken(userId) {
...
export async function getDeleteToken(userId) {
...
// Revokes a token.
export async function revokeDeleteToken(userId) {
    await DeleteToken.updateOne({ userId }, {
        revoked: true
    });
}
