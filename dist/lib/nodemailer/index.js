...
import { WELCOME_EMAIL_TEMPLATE, WELCOME_VERIFY_EMAIL_TEMPLATE, NEWS_SUMMARY_EMAIL_TEMPLATE, RESET_PASSWORD_EMAIL_TEMPLATE } from './templates.js';
...
export const sendWelcomeVerifyEmail = async ({ email, name, intro, verificationUrl, deleteUrl }) => {
...
export const sendNewsSummaryEmail = async ({ email, date, newsContent, deleteUrl }) => {
    let htmlTemplate = NEWS_SUMMARY_EMAIL_TEMPLATE.replace('{{date}}', date).replace('{{newsContent}}', newsContent);
    if (deleteUrl) {
...
export const sendResetPasswordEmail = async ({ email, name, resetPasswordUrl }) => {
    const htmlTemplate = RESET_PASSWORD_EMAIL_TEMPLATE.replaceAll('{{name}}', name).replaceAll('{{resetPasswordUrl}}', resetPasswordUrl);
    const mailOptions = {
        from: 'Real Time Stock Market',
        to: email,
